...
public class GererAnnonceAudio{
	...
	public String demanderFlux(String message, DataObject configAudioTmp, DataObject configHttpTmp) throws AppTechnicalException{
		try{
			...
			if(!download){
				...
				logger.log(LogService.LOG_ERROR,  ex.getMessage());
				throw new AppTechnicalException(CoreUtil.ERREUR_HTTP_DOWNLOAD, ex);
			}else if(!encode){	
 
				logger.log(LogService.LOG_ERROR,  ex.getMessage());
				throw new AppTechnicalException(CoreUtil.ERREUR_AUDIO_ENCODING, ex);
			}else{
 
				logger.log(LogService.LOG_ERROR,  ex.getMessage());
				throw new AppTechnicalException(CoreUtil.ERREUR_HTTP_UPLOAD, ex);
			}
		}catch (ObixTmlgExeption e) {
 
			logger.log(LogService.LOG_ERROR,  e.getMessage());
			throw new AppTechnicalException(CoreUtil.ERREUR_FORMAT_OBIX);
		} catch (Exception e) {
			logger.log(LogService.LOG_ERROR,  e.getMessage());
			throw new AppTechnicalException(CoreUtil.ERREUR_HTTP_CONFIG, e);
		}
	}
	...
	public AudioFile traiterAnnonceAudio(IFluxTmlg flux, IAnnonceAudioTmlg annonceAudio) throws CoreException, Exception{
		...
		AudioFile newAudioFile = CoreUtil.simulateEncodedAudioFileProperties(fileName, configAudio.getString("final_audio_file_extension"));
		...
		if(verifAudioFileExistsOnServer(localServerFolder, encodedFilename)){
			...
			if(encodedAudioFile.exists()){
				...
				newAudioFile = CoreUtil.encodeAudioFile(audioTemporalyPath, fileName, configAudio);
				encode = true;
			}
